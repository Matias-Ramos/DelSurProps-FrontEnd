...
const defaultDotStatuses = {
  ...
  function manageHover(colorStatuses, action) {
    ...
    switch (action.type) {
      ...
      default:
        console.error("error on NavBtns.jsx > colorModifier fn > switch statement, when hovering over one of the navBar btns");
  ...
  function manageClick(action) {
    ...
    switch (action.type) {
      ...
      default:
        console.error("error on NavBtns.jsx > colorModifier fn > switch statement, when clicking one of the navBar btns");
        return defaultDotStatuses;
    }
  }
  function colorModifier(colorStatuses, action) {
    if (action.hover) 
      return manageHover(colorStatuses, action)
    else 
      return manageClick(action)
  }
